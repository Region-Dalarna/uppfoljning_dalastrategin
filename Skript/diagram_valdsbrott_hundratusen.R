diagram_valdsbrott_hundratusen <- function(region_vekt = "20",
                                            output_mapp = "G:/Samhällsanalys/Statistik/Näringsliv/basfakta/",
                                            returnera_data = FALSE,
                                            kpi = c("N07549", "N07537","N07403"), # Ekobrott, finns även andra, se Kolada
  ...
  valdsbrott_hundratusen_df <- hamta_kolada_df(kpi =  kpi,hamta_kommunkoder(region_vekt)$regionkod,valda_ar = c(2000:2100))
  
  # Väljer bort variabler och ger mer rimliga namn.
  valdsbrott_hundratusen_df <- valdsbrott_hundratusen_df %>%
    select(ar,region,variabelkod,variabel,varde) %>% 
    mutate(region = skapa_kortnamn_lan(region,byt_ut_riket_mot_sverige = TRUE))
  
  
  if(returnera_data == TRUE){
    assign("valdsbrott_hundratusen_df", valdsbrott_hundratusen_df, envir = .GlobalEnv)
  }
  ...
  diag <- map(kpi, ~ skapa_diagram(valdsbrott_hundratusen_df, .x)) %>% flatten()
