diagram_sjukdomar_hundratusen <- function(region_vekt = "20",
                                      output_mapp = "G:/Samhällsanalys/Statistik/Näringsliv/basfakta/",
                                      returnera_data = FALSE,
                                      kpi = c("N60920","U20462"), # Hjärtinfarkt
  ...
  sjukdomar_hundratusen_df <- hamta_kolada_df(kpi = kpi,hamtaAllaLan(),valda_ar = c(2000:2100))
  
  # Väljer bort variabler och ger mer rimliga namn.
  sjukdomar_hundratusen_df <- sjukdomar_hundratusen_df %>%
    select(ar,region,kon,variabelkod,variabel,varde) %>% 
    mutate(region = skapa_kortnamn_lan(region,byt_ut_riket_mot_sverige = TRUE))
  
  
  if(returnera_data == TRUE){
    assign("sjukdomar_hundratusen_df", sjukdomar_hundratusen_df, envir = .GlobalEnv)
  }
  ...
  diag <- map(kpi, ~ skapa_diagram(sjukdomar_hundratusen_df, .x)) %>% flatten()
